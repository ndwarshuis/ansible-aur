...
packages:
    description: a list of packages installed
"""
...
def upgrade(module, use, extra_args, aur_only, update_cache):
    ...
    _, out, _ = module.run_command(command, check_rc=True)

    packages = []
    regex = re.compile(r"([\w+\-.@]+)\s+(\S+-\S+)\s+->\s+(\S+-\S+)")
    lines = out.split("\n")
    lines.remove("")
    for pkg in lines:
        matched = regex.search(pkg)
        if matched is not None:
            packages.append(matched.group(1))

    is_changed = len(packages) > 0

    message = "upgraded system" if is_changed else "nothing to upgrade"
